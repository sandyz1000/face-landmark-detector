...
def save_img_landmark(row, index, height, width, is_training_set=False, prefix='kaggle'):
...
def prepare_and_save(fname, img_shape=(96, 96), is_train=True, prefix="kaggle"):
    ...
    df['Image'] = df['Image'].apply(lambda im: np.fromstring(im, sep=' ')).astype(np.float32)
    ...
    for index, row in enumerate(df.iterrows()):
        save_img_landmark(
            ...
            is_training_set=is_train, prefix=prefix
        )
