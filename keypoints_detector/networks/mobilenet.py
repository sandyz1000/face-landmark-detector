...
def _conv_block(inputs, filters, alpha, kernel=(3, 3), strides=(1, 1)):
    ...
    x = KL.ZeroPadding2D(padding=(1, 1), name='conv1_pad',
                         data_format=IMAGE_ORDERING)(inputs)
    x = KL.Conv2D(filters, kernel, data_format=IMAGE_ORDERING,
                  padding='valid',
                  use_bias=False,
                  strides=strides,
                  name='conv1')(x)
...
def _depthwise_conv_block(inputs, pointwise_conv_filters, alpha,
    ...
    x = KL.ZeroPadding2D((1, 1), data_format=IMAGE_ORDERING,
                         name='conv_pad_%d' % block_id)(inputs)
    x = KL.DepthwiseConv2D((3, 3), data_format=IMAGE_ORDERING,
                           padding='valid',
    ...
    x = KL.Conv2D(pointwise_conv_filters, (1, 1), data_format=IMAGE_ORDERING,
                  padding='same',
                  use_bias=False,
                  strides=(1, 1),
