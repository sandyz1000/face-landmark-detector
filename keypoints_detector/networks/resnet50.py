...
def one_side_pad(x):
    x = KL.ZeroPadding2D((1, 1), data_format=IMAGE_ORDERING)(x)
    if IMAGE_ORDERING == 'channels_first':
        x = KL.Lambda(lambda x: x[:, :, :-1, :-1])(x)
    elif IMAGE_ORDERING == 'channels_last':
        x = KL.Lambda(lambda x: x[:, :-1, :-1, :])(x)
    return x
...
def identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    x = KL.Conv2D(filters1, (1, 1), data_format=IMAGE_ORDERING,
                  name=conv_name_base + '2a')(input_tensor)
    x = KL.BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = KL.Activation('relu')(x)

    x = KL.Conv2D(filters2, kernel_size, data_format=IMAGE_ORDERING,
                  padding='same', name=conv_name_base + '2b')(x)
    x = KL.BatchNormalization(axis=bn_axis, name=bn_name_base + '2b')(x)
    x = KL.Activation('relu')(x)

    x = KL.Conv2D(filters3, (1, 1), data_format=IMAGE_ORDERING,
...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = KL.Conv2D(filters1, (1, 1), data_format=IMAGE_ORDERING, strides=strides,
                  name=conv_name_base + '2a')(input_tensor)
    x = KL.BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = KL.Activation('relu')(x)
    ...
    shortcut = KL.Conv2D(filters3, (1, 1), data_format=IMAGE_ORDERING,
...
def get_resnet50_encoder(input_height=224, input_width=224,
    ...
    x = KL.ZeroPadding2D((3, 3), data_format=IMAGE_ORDERING)(img_input)
    x = KL.Conv2D(64, (7, 7), data_format=IMAGE_ORDERING,
                  strides=(2, 2), name='conv1')(x)
    f1 = x

    x = KL.BatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    x = KL.Activation('relu')(x)
    x = KL.MaxPooling2D((3, 3), data_format=IMAGE_ORDERING, strides=(2, 2))(x)
    ...
    x = KL.AveragePooling2D((7, 7), data_format=IMAGE_ORDERING, name='avg_pool')(x)
    # f6 = x
